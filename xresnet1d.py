...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        out = self.act(self.convpath(x) + self.idpath(x))
...
class XResNet1d(nn.Sequential):
    ...
    def __init__(self, block, expansion, layers, p=0.0, input_channels=3, num_classes=1000, stem_szs=(32, 32, 64),
                 ...
                 widen=1.0, sa=False, headless=True, act_cls=nn.ReLU, lin_ftrs_head=None, ps_head=0.5,
        ...
        if headless:  # [32, 256, 32] -> [32, 256]
            ...
            #     head,
            # )
            fc = nn.Linear(in_features=256, out_features=num_classes)
            head = nn.Sequential(nn.AdaptiveAvgPool1d(1), nn.Flatten())
            super().__init__(
                *stem, nn.MaxPool1d(kernel_size=3, stride=2, padding=1),
                *blocks,
    ...
    # def forward(self, x):
    #     for i, module in enumerate(self[-2].modules()):
    #         x = module(x)
    #         if isinstance(module, ResBlock):
    #             print(f"Output after ResBlock {i}: {x.shape}")
    #     return x
...
def ecg_inference(input_data):
    ...
    lead = input_data.shape[1]
    current_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(current_dir, "sl_model_12.pt")

    if lead == 12:
        model = xresnet1d101(num_classes=4, input_channels=12, headless=False)
        chkpoint = torch.load(model_path, map_location='cpu')
        idx_to_class = {0: 'AFIB-Atrial Fibrillation', 1: 'GSVT', 2: 'SB-Sinus Bradycardia', 3: 'SR-Sinus Rhythm'}
    else:
        model = xresnet1d50(num_classes=4, input_channels=1, headless=False)
        chkpoint = torch.load(model_path, map_location='cpu')
        idx_to_class = {0: 'Normal', 1: 'AF', 2: 'Other rhythm', 3: 'Noisy'}
    ...
    predicted_class_name = idx_to_class[predicted_class[0]]
    return lead, predicted_class_name


if __name__ == '__main__':
    input_shape = (12, 1000)  # 假设输入维度为 (通道数, 输入长度)
    model = xresnet1d101(num_classes=4, input_channels=12, headless=False).cuda()
    chkpoint = torch.load("sl_model_12.pt", map_location='cuda:0')
    pretrained_dict = chkpoint["model_state_dict"]
    ...
    _, predicted_class = torch.max(output, 1)  # 返回每个样本的最大值索引
    predicted_class = predicted_class.cpu().numpy()
    print("Predicted class index:", predicted_class)
    idx_to_class = {0: 'Class A', 1: 'Class B', 2: 'Class C', 3: 'Class D', 4: 'Class E'}
    predicted_class_name = idx_to_class[predicted_class[0]]
    print("Predicted model diagnosis:", predicted_class_name)
